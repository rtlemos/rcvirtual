...
rcrandom.abstract <- setRefClass(
    ...
    methods = list(
        ...
        set.name = function(new.name){
            ...
            print(paste0("Changed the name of ", class(.self),
                         ", from '", .self$name,
                         "' to '", new.name,"'"),
                  quote = FALSE)
            .self$name <- new.name
        ...
        get.name = function(){
            ...
            print(paste0("Name of ", class(.self), ": '", .self$name, "'"),
                  quote = FALSE)
        },
        ...
        get.projection  = function(two.vars){
            "Projects variables onto a coordinate system of 2 anti-variables,
            if they exist."
            ...
            pj <- t(matrix(nrow = 4,ncol = .self$np,
        ...
        get.bivariate = function(two.vars){
            "Provides data to plot the posterior distribution of variables"

            coord <- expand.grid(lapply(1:2, FUN = function(j) {
                post  <- .self$get.marginal(id = two.vars[j], keep.log = TRUE)
                my.np <- length(post$x)
                sqnp  <- round(sqrt(my.np))
                endp  <- post$x[c(1,my.np)]
                return(seq(endp[1], endp[2], length = sqnp))
            }))
            npoints <- nrow(coord)
            myZ <- .self$get.Z()
            ...
            density <- mapply(1:npoints, FUN = function(k){
                idx <- .self$iget.index(
                    ...
                    spac = spac,
                    znp = znp)
                dens <- exp(sum(mapply(1:.self$np, FUN = function(o){
                    zpost$y[idx[o], o]
                })))
                return(dens)
            })
            my.biv  <- data.frame(x = coord[, 1], y = coord[, 2], z = density)
            return(my.biv)
        },

        parameter = function(id, value = NULL, eval = TRUE){
            "Interface to read (value=NULL) or write (value not NULL)
            the object's parameters"

            #callSuper(...)
            if (missing(value)) {
                return(.self$iget.parameter(id = id, eval = eval))
            } else {
        ...
        show = function(){
            ...
            sgv <- unlist(lapply(1:npr, function(i) (length(val[[i]]) == 1)))
            if (all(sgv)) {
                hd  <- paste0(typ, .self$type, " ( ")
                ft  <- " )\n"
                cre <- paste0(pn[1]," = ",val[[1]])
                if (npr > 1) {
                    cre <- c(cre, unlist(mapply(2:npr, FUN = function(i){
                        paste0( ", ", pn[i]," = ",val[[i]])
                    })))
                }
                cat(hd, cre, ft, sep = "")
        ...
        iset.parameter = function(id, value){
            ...
            if (.self$type == "Constant") {
                if (is(value, "RAT")) {
                    ...
                    re <- value$iget.parameter(1, eval = FALSE)
                    sz <- value$size()
                } else {
                    rv <- value
                    ...
                    if (is(value, "numeric")) {
                        ...
                        sz <- list(nr = nrow(value), nc = ncol(value))
                    } else {
                        stop("iset.parameter: unknown data type")
                    }
                }
        ...
        iset.dexpr = function(id, deparsed.expr){
            "Sets the deparsed expression used to construct the object's
            parameters (e.g., mean, variance)"

            .self$dexpr[[id]] <- deparsed.expr
        },

        iset.operate = function(operation, operand, operand.name, operand.side,
                                my.name, ...){
            print("Should be replaced with offspring method")
        },

        iget.parameter = function(id, eval=TRUE, ...){
            "Provides the random variable's parameter value (eval=T),
            or its deparsed expression (eval=F)"

            myexpr <- .self$dexpr[[id]]
            if (eval) {
                if (.self$type == "Constant") {
                    out <- mapply(seq_along(myexpr), FUN = function(i) {
                    ...
                    if (!is(out, "RAT")) {
                        if (.self$nc == 1) {
                            ...
                            out <- matrix(nrow = .self$nr, ncold = .self$nc,
                                          as.numeric(out))
                        }
                    }
                } else {
                    out <- lapply(seq_along(myexpr), FUN = function(i){
                        eval(parse(text = myexpr[i]))
                    })
                    if (length(out) == 1) out <- out[[1]]
        ...
        iget.optimization.parameters = function(){
            "This function should be replaced with another that does not
            return NULL, when auxiliary parameters exist."
            return(NULL)
        },
        ...
        iget.Z = function(){
            "Provides the associated z-transformed random variable or vector."
        ...
        iget.convert = function(x, j = NULL){
            "Forward-transformation (i.e., from X to Z) of the vector x,
            or just one element of it."

            if (missing(j)) {
                z <- .self$mode$precR %*% (x - .self$mode$par)
            } else {
                precRvec <- .self$mode$precR[,j]
                modej <- .self$mode$par[j]
                z <- t(mapply(x, FUN = function(x) precRvec * (x - modej)))
            }
            return(z)
        },

        iget.zconvert = function(z, j = NULL){
            "Back-transformation (i.e., from Z to X) of the vector z,
            or just one element of it."
            ...
            if (missing(j)) {
                ...
                parj <- .self$mode$par
                x <- t(mapply(z, FUN = function(z) covRcol * z + parj ))
            }
            return(x)
        },
        ...
        ## Not sure if iget.index and iget.zindex are still needed
        iget.index = function(two.vars, coords, lb, first.zc, spac, znp){
            "Provides the indices of the 2 z-variables closest to
            the input coordinates"

            my.vec <- rep(0,.self$np)
            my.vec[two.vars[1]] <- coords[1] - .self$specs$par[two.vars[1]]
            my.vec[two.vars[2]] <- coords[2] - .self$specs$par[two.vars[2]]
            un <- .self$specs$precR %*% my.vec
            idx <- round( (un - lb) / spac) + 1 #differs from below ?!
            idx[idx < 1] <- 1
            idx[idx > znp] <- znp
            return(idx)
        },

        iget.zindex = function(two.vars, coords, lb, first.zc, spac, znp){
            "Provides the indices of the 2 z-variables closest to the
            input coordinates"
